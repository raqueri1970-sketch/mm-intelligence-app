...
export async function GET() {
  ...
  if (!key) return NextResponse.json({ ok: false, error: 'Supabase server key ausente' }, { status: 500 })
  try {
    ...
    return NextResponse.json({ ok: false, error: e?.message || 'tiktok_status_failed' }, { status: 500 })
  }
}
