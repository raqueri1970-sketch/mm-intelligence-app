...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <aside className={collapsed ? 'mm-sidebar mm-sidebar--collapsed' : 'mm-sidebar'}>
      <div className="mm-logo"><div className="mm-logo__mark">MM</div>{!collapsed&&<div className="mm-logo__text"><span className="mm-logo__name">Intelligence</span><span className="mm-logo__tag">Drop · AI · Score</span></div>}</div>
      <nav className="mm-nav">{nav.map(({href,label,icon,externalHref})=>{const active=path===href;return externalHref?<div key={href} style={{display:'flex',alignItems:'center'}}><Link href={href} className={'mm-nav__item'+(active?' mm-nav__item--active':'')} style={{flex:1}}><span className="mm-nav__icon">{icon}</span>{!collapsed&&<span className="mm-nav__label">{label}</span>}{active&&<span className="mm-nav__active-bar"/>}</Link>{!collapsed&&<a href={externalHref} target="_blank" rel="noopener noreferrer" style={{padding:'6px 8px',color:'var(--text3)',fontSize:12}}>↗</a>}</div>:<Link key={href} href={href} className={'mm-nav__item'+(active?' mm-nav__item--active':'')}><span className="mm-nav__icon">{icon}</span>{!collapsed&&<span className="mm-nav__label">{label}</span>}{active&&<span className="mm-nav__active-bar"/>}</Link>})}</nav>
      {!collapsed&&<div className="mm-lang"><span className="mm-lang__label">Idioma</span><div className="mm-lang__pills">{[['pt','🇧🇷 PT'],['en','🇺🇸 EN'],['es','🇲🇽 ES']].map(([l,label])=><button key={l} onClick={()=>setLang(l)} className={'mm-lang__pill'+(lang===l?' mm-lang__pill--active':'')}>{label}</button>)}</div></div>}
      <button className="mm-collapse" onClick={()=>setCollapsed(!collapsed)}>{collapsed?'›':'‹'}</button>
    </aside>
